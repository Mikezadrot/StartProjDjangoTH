...
def create_table_db(table_name, fields_and_types):
    ...
    # # print("maybe it`s ok")
    # try:
    #     # Параметри підключення до PostgreSQL бази даних
    #
    #     # З'єднання з базою даних
    #     conn = connect_to_db()
    #     cursor = conn.cursor()
    #
    #     # Отримуємо існуючі поля таблиці
    #     cursor.execute(
    #         f"SELECT column_name, data_type FROM information_schema.columns WHERE table_name = '{table_name}';")
    #     existing_columns = {row[0]: row[1] for row in cursor.fetchall()}
    #
    #     # Генеруємо SQL-запити на зміну структури таблиці
    #     alter_table_queries = []
    #
    #     for field, field_type in fields_and_types.items():
    #         if field in existing_columns:
    #             if existing_columns[field] != field_type:
    #                 # Якщо тип даних поля не співпадає, генеруємо SQL-запит на його зміну
    #                 alter_query = f"ALTER TABLE {table_name} ALTER COLUMN {field} TYPE {field_type};"
    #                 alter_table_queries.append(alter_query)
    #         else:
    #             # Якщо поле не існує в таблиці, генеруємо SQL-запит на його додавання
    #             add_query = f"ALTER TABLE {table_name} ADD COLUMN {field} {field_type};"
    #             alter_table_queries.append(add_query)
    #
    #     # Виконуємо зміни в структурі таблиці
    ...
    #     cursor.close()
    #     conn.close()
    #     logging.info(f"Table: {table_name} was created or updated")
    #
    # except Exception as e:
    #     logging.info(f"Помилка при оновленні таблиці: {e}")
